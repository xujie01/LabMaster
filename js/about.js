...
Global.reCal = function(){
    ...
    $('.section-content').each(function(){
        $(this).css({
            ...
            marginTop: $(this).hasClass("first")?170:80
        });
    });
};
//处理首页背景图片，填充满
...
$(document).ready(function() {
    ...
    $('.nav a').click(function(e) {
        ...
        if ( target == 1 ) {
            window.open("img/about/徐杰-武汉大学-硕士-测绘工程.pdf");
            Global.shrinkHeader(false);
            $('.nav .fade').removeClass('hover');
        }else{
            if(target>=3){
                Global.fire_nav(target-1);
            }else{
...
Global.scrollHandle = function(scrollDown){
    if (!Global.isScrolling) {
        ...
        if(Global.section_num>=3){
            Global.fire_nav(Global.section_num-1);
        }else{
